 ...
 * @returns
 */
const getPublishableResults = (
    result: BACReadMultiple,
    ...
    for (const valueGroup of result.values) {
        ...
        const instanceName: string = getNameFromInstance(
            instance,
        ...
        for (const value of valueGroup.values) {
            ...
            for (const subValue of value.value) {
                ...
                publishableResults.push({
                    topic: `${instanceName}/${propertyIdentifier
                        .toString()
                        .toLowerCase()}`,
                    value: dataValue,
                });
